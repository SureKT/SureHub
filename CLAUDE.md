...
- **Capa LLM vía LiteLLM** — `app/services/llm.py` no llama a la SDK de Anthropic directo; pasa por `app/services/llm_router.py` (tier→modelo + fallback) y cada llamada se loguea en `llm_calls` (SQLite). Tiers: `cloud` (sonnet) y `local_ok` (Ollama `qwen2.5:3b` local → fallback Haiku). Spec: `docs/superpowers/specs/2026-06-26-llm-routing-evals-design.md`
- **LLM local (Fase 2, 2026-08-25)** — container `ollama` en el server (CPU, red Docker `ollama-net`, sin puerto en LAN). El bot lo usa para `complete_tags` (tags de notas + clasificación de inbox) si `OLLAMA_BASE_URL` está definido; si no, o si Ollama falla/tarda más de `LLM_LOCAL_TIMEOUT`, cae a Haiku y se loguea `fell_back=true`. El fallback se hace en `llm_router.complete()`, no con `fallbacks=` de litellm (ese path reusa `api_base` y rompería el reintento cloud). Latencia medida en CPU: tags ~1-1,5 s, clasificación ~2-3 s. Env: `OLLAMA_BASE_URL`, `OLLAMA_MODEL`, `LLM_LOCAL_TIMEOUT`. Infra: `SureKT/homelab` → `docs/ollama.md`
